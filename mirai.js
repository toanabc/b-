...
try {
    ...
    else return logger.loader("config.json không tìm thấy!", "error");
}
...
catch { return logger.loader("Không thể tải cấu hình tệp!", "error") }
...
//if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[ BAN TOÀN CẦU ]\n\n');
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '[ Dữ liệu ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ Dữ liệu ]'); }
})();
